...
CREATE TRIGGER IF NOT EXISTS `dev_siak_krs_nilai_update` AFTER UPDATE ON `dev_siak_krs` FOR EACH ROW
    UPDATE `dev_siak_nilai` SET tahun_ajaran = NEW.tahun_ajaran WHERE `dev_siak_nilai`.`krs_id` = OLD.id;
...
SELECT COUNT(*) FROM `dev_siak_nilai` INTO n;
SET i=0;
WHILE i<=n DO

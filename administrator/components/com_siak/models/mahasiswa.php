...
class SiakModelMahasiswa extends JModelAdmin
{
    protected $_item;
    ...
    public function __construct($config)
    {
        $config = array_merge(
            [
                'event_after_delete' => 'onMahasiswaAfterDelete',
                'event_after_save' => 'onMahasiswaAfterSave',
                'event_before_delete' => 'onMahasiswaBeforeDelete',
                'event_before_save' => 'onMahasiswaBeforeSave',
            ],
            $config
        );
    ...
    {
        $form = $this->loadForm('com_siak.mahasiswa', 'mahasiswa', ['control' => 'jform', 'load_data' => $loadData]);
        if (empty($form)) {
            return false;
        }
    ...
    {
        $data = JFactory::getApplication()->getUserState('com_siak.edit.mahasiswa.data', []);
        if (empty($data)) {
            $data = $this->getItem();
        }
    ...
    {
        $pk = (!empty($pk)) ? $pk : (int) $this->getState('mahasiswa.id');
        ...
        $uid = JFactory::getApplication()->getUserState('com_siak.edit.mahasiswa.userID', '0');
    ...
    {
        $pk = (!empty($data['id'])) ? $data['id'] : (int) $this->getState('mahasiswa.id');
        $table = $this->getTable();
        $table->load($pk);
        if (!$table->bind($data)) {
    ...
    public function getRecordID($uid)
    {
        ...
        $query->select('id');
        $query->from($db->qn('#__siak_mhs_status'));

...
class SiakModelIndustri extends JModelAdmin
{
    protected $_item;
    ...
    public function __construct($config)
    {
        $config = array_merge(
            [
                'event_after_delete' => 'onIndustriAfterDelete',
                'event_after_save' => 'onIndustriAfterSave',
                'event_before_delete' => 'onIndustriBeforeDelete',
                'event_before_save' => 'onIndustriBeforeSave',
            ],
            $config
        );
    ...
    public function getTable($type = 'Industri', $prefix = 'Table', $config = [])
    {
    ...
    {
        $form = $this->loadForm($this->context, 'industri', ['control' => 'jform', 'load_data' => $loadData]);
        if (empty($form)) {
            return false;
        }
    ...
    {
        $data = JFactory::getApplication()->getUserState('com_siak.edit.industri.data', []);
        if (empty($data)) {
            $data = $this->getItem();
        }
    ...
    {
        $pk = (!empty($pk)) ? $pk : (int) $this->getState('industri.id');
    ...
    {
        $pk = (!empty($data['id'])) ? $data['id'] : (int) $this->getState('industri.id');
        ...
        foreach ($pks as $i => $pk) {
            if ($table->load($pk)) {
                if ($this->canDelete($table)) {
                    ...
                    Siak::deleteBerkas($item->dokumen_kerjasama, 'industri');
